...
export default function PriceFilter({ products, filteredProductsByPrice, setFilteredProductsByPrice, localLang, localCurrency, setNoMatchToInputtedPriceRange, setFilteredProductsByInputtedPriceRange, filteredProductsByStock, setFilterPopup }: PriceFilterProps) {
    ...
    const priceRangeFilter = (from: number, to?: number) => {
        if (to) {
            return products.filter((prod) => {
                return prod.salePrice ? prod.salePrice[localCurrency] >= from && prod.salePrice[localCurrency] < to : prod.price[localCurrency] >= from && prod.price[localCurrency] < to;
            });
        } else {
            return products.filter((prod) => {
    ...
    const numOfProductsByFilter = (filter: ProductItem[]) => {
        return filteredProductsByStock.length > 0 ? filteredProductsByStock.filter((prod) => filter.includes(prod)).length : products.filter((prod) => filter.includes(prod)).length;
    };
    ...
    const priceRanges = [
        {
            ...
            filterFunc: () => filterByPrice(priceRangeFilter(0, 200), setIsPR_lessThan200_Checked, isPR_lessThan200_Checked),
    ...
    const filterByPrice = (prodsInPriceRange: ProductItem[], setChecked: Function, isChecked: boolean) => {
        if (isChecked) {
            ...
            setFilteredProductsByPrice([...filteredProductsByPrice, ...prodsInPriceRange.filter((prod) => !filteredProductsByPrice.includes(prod))]);
        }
    };
        ...
        <div className="">
            ...
            <div className="flex flex-col py-4 border-b px-1">
                ...
                <button
                    ...
                    className={`${disabledApplyPriceRangeButton ? "hidden" : ""} text-sm text-primary pt-6 self-start hover:underline`}
                >
