...
export default function SearchBar({ searchBarInput, search, searchResults, query }: SearchBarProps) {
	...
	const localLang: any = localStorage.locality ? (localStorage.locality === "Canada (FR)" ? localityText.french : localityText.english) : localityText.english;
	
	const lang = localStorage.locality ?
		(localStorage.locality === "Canada (FR)" ? "french" : "english")
		: "english";
		...
		if (queryInput != "") {
			...
			queryInputArr.forEach(queryTerm => {
				sitePages.map((page: {english: string, french: string}) => {
					if (page[lang].toLowerCase().includes(queryTerm.toLowerCase()) && !pagesArr.includes({name: page, link: page["english"].toLowerCase()})) {
			...
			queryInputArr.forEach(queryTerm => {
				allBlogs.map((blog: Blog) => {
					const title = blog.title[lang].toLowerCase();
					const desc = blog.desc[lang].toLowerCase();
					const queryLC = queryTerm.toLowerCase();
					if (title.includes(queryLC) || desc.includes(queryLC) && !pagesArr.includes({name: blog.title, link: `blog/${encodeURIComponent(blog.title["english"])}`})) {
	...
	return (
		<div id="searchBar" className="relative w-[42rem] pointer-events-auto group/searchbarAndResults">
			...
			<form action={`/search/${queryDisplay}`} className={`relative ${search ? "ring-1 ring-neutral-500 group-hover/searchbarAndResults:ring-2 transition duration-200" : "ring-0"} w-full flex`}>
				...
				<label htmlFor="searchBarInput" className={`absolute left-6 ${queryDisplay != "" ? "top-1 text-[10px] duration-0" : "top-3.5 peer-focus:top-1 peer-focus:text-[10px]"} text-neutral-500 pointer-events-none transition-all duration-200`}>{ localLang.nav_search }</label>
			...
			<div className={`absolute ${(queryDisplay != "" && searchResultsDisplay) ? "ring-1 ring-neutral-500 group-hover/searchbarAndResults:ring-2 transition duration-200" : "hidden" } top-13 z-10 w-full h-auto bg-white overflow-hidden`}>
				<div className="flex gap-4">
					<div className={`flex flex-col w-2/5 ${suggestions.length > 0 || pages.length > 0 ? "py-2" : ""}`}>
						<div className={`flex flex-col ${suggestions.length > 0 ? "" : "hidden"}`}>
							<p className="text-[11px] uppercase text-primary py-2 mx-6 border-b border-primary/50 tracking-wider">{ localLang.search_suggestions }</p>
							<div className="">
						...
						<div className={`flex flex-col ${pages.length > 0 ? "" : "hidden"}`}>
							<p className="text-[11px] uppercase text-primary py-2 mx-6 border-b border-primary/50 tracking-wider">{ localLang.search_pages }</p>
							<div className="">
								{   /* list all pages with query bolded */
									pages.map((page, index) => (
										<a href={`/${page.link}`} key={index}>
											<p className="text-sm px-6 py-2 hover:bg-black/10 transition duration-100">
												{boldQueryInSearchResult(page.name[lang])}	
											</p>
										</a>
									))
								}
					...
					<div className={`flex flex-col w-3/5 ${products.length > 0 ? "py-2" : "hidden"}`}>
						<p className="text-[11px] uppercase text-primary py-2 mx-6 border-b border-primary/50 tracking-wider">{ localLang.search_products }</p>
						<div className="">
							{
								products.map((prod, index) => (
									<a href={`/product/${prod.name}`} key={index} className="flex items-center gap-6 px-6 py-2 hover:bg-black/10 transition duration-100">
				...
				<div className={`${(queryDisplay != "" && searchResultsDisplay) ? "" : "hidden"}`}>
					<a href={`/search/${queryDisplay}`} className="flex border-t justify-between items-center px-6 py-2 group/toSearchPage hover:bg-black/10">
						<p className="text-sm">{ localLang.search_search_for } "{queryDisplay}"</p>
