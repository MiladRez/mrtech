        ...
        if (queryInput != "") {
            ...
            queryInputArr.forEach((queryTerm) => {
                sitePages.map((page: { english: string; french: string }) => {
                    if (page[localLang.lang].toLowerCase().includes(queryTerm.toLowerCase()) && !pagesArr.includes({ name: page, link: page["english"].toLowerCase() })) {
                ...
                <div className="flex flex-col sm:flex-row sm:gap-4">
                    <div className={`flex flex-col sm:w-2/5 ${suggestions.length > 0 || pages.length > 0 ? "pt-2 sm:py-2" : ""}`}>
                            ...
                            <div className="">
                                {
                                    ...
                                    suggestions.map((suggestion, index) => (
                                        <a href={`/search/${suggestion}`} key={index}>
                                            <p className="text-sm px-6 py-2 hover:bg-black/10 transition duration-100">{boldQueryInSearchResult(suggestion)}</p>
                                        </a>
                                    ))
                                }
                            </div>
                        ...
                        <div className={`flex flex-col ${pages.length > 0 ? "" : "hidden"}`}>
                            <p className="text-[11px] uppercase text-primary py-2 mx-6 border-b border-primary/50 tracking-wider">{localLang.text.search_pages}</p>
                            <div className="">
                                {
                                    /* list all pages with query bolded */
                                    pages.map((page, index) => (
                                        <a href={`/${page.link}`} key={index}>
                                            <p className="text-sm px-6 py-2 hover:bg-black/10 transition duration-100">{boldQueryInSearchResult(page.name[localLang.lang])}</p>
                                        </a>
                                    ))
                                }
                            </div>
