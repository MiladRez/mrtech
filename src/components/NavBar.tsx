...
export default function NavBar({ product, setProduct }: { product?: ProductItem | null, setProduct?: Function}) {
	...
	return (
		<>
			<nav className={`fixed ${visible ? "max-h-24" : "max-h-0 overflow-hidden"} w-screen flex justify-center bg-stone-50 border-b z-20 transition-[max-height] duration-300`}>
				<div className="relative flex w-full max-w-screen-xl justify-between px-12 py-4">
					<div className="absolute left-0 top-0 w-full h-full px-12 py-4 flex justify-end pointer-events-none">
						{
							product && product.id ?
							<div id="cartPopup" className={`fixed ${visible ? "top-24" : "top-0"} border w-96 bg-white transition-[top] duration-300 pointer-events-auto`}>
								<div className="relative flex flex-col gap-2 px-12 py-8">
									...
									<div className="flex items-center">
										<img src={ product.img.toString() } className="w-28 px-4 py-4" />
			...
			{/* search bar */}
			<section id="searchBar" className={`fixed ${search ? "max-h-24" : "max-h-0"} w-screen h-24 flex justify-center items-center bg-neutral-50 z-10 transition-[max-height] duration-200 overflow-hidden`}>
			...
			<section className={`${search ? "bg-black pointer-events-auto" : ""} w-screen h-screen fixed opacity-50 z-10 pointer-events-none`}></section>
		</>
	)
}